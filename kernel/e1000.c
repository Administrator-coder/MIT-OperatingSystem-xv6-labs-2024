...
e1000_transmit(char *buf, int len)
{
  ...
  // mark descriptor as in-use; device will set DD when done
...
e1000_intr(void)
{
  ...
  // free any completed transmit buffers so memory doesn't leak
  acquire(&e1000_lock);
  for(int i = 0; i < TX_RING_SIZE; i++){
    if(tx_bufs[i] && (tx_ring[i].status & E1000_TXD_STAT_DD)){
      kfree(tx_bufs[i]);
      tx_bufs[i] = 0;
    }
  }
